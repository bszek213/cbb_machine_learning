...
#TODO: Create a PCA method that remove correlated features and reduces the dimension so that the resulting dims = 95% of the total variance
...
class cbbDeep():
    ...
    def create_model(self, neurons=32, learning_rate=0.001, dropout_rate=0.2, alpha=0.1):
        ...
        return model
    def deep_learn(self):
        if exists('deep_learning_MA.h5'):
            self.model = keras.models.load_model('deep_learning_MA.h5')
        else:
            #best params
            ...
            # input()
            self.model.save('deep_learning_MA.h5')
            plt.figure()
    ...
    def predict_two_teams(self):
        ...
        while True:
            try:
                ...
                #replace team 1 opp data with teams 2 data
                data1_mean_copy = data1_mean
                for col in data1_mean.columns:
                        if "opp" in col:
                            if "opp_trb" in col:
                                new_col = col.replace("opp_trb", "total_board")
                ...
                data2_mean.loc[data2_mean.index[-1], 'simple_rating_system'] = cbb_web_scraper.get_latest_srs(team_2)
                data1_mean_copy.loc[data1_mean_copy.index[-1], 'simple_rating_system'] = team_1_srs
                #TEAM 1 Prediction
                x_new = self.scaler.transform(data1_mean.iloc[-1:])
                prediction_team_1 = self.model.predict(x_new)
                #replace team 2 opp data with teams 1 data
                for col in data2_mean.columns:
                        if "opp" in col:
                            if "opp_trb" in col:
                                new_col = col.replace("opp_trb", "total_board")
                                data2_mean.loc[data2_mean.index[-1], col] = data1_mean_copy.loc[data1_mean_copy.index[-1], new_col]
                            else:
                                new_col = col.replace("opp_", "")
                                data2_mean.loc[data2_mean.index[-1], col] = data1_mean_copy.loc[data1_mean_copy.index[-1], new_col]
                #TEAM 2 Prediction
                x_new = self.scaler.transform(data2_mean.iloc[-1:])
                prediction_team_2 = self.model.predict(x_new)
                print('===============================')
                print(f'prediction of {team_1} winning: {prediction_team_1[0][0]*100}%')
                print(f'prediction of {team_2} winning: {prediction_team_2[0][0]*100}%')
                print('===============================')
